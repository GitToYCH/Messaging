...
class SolarDate extends MyDate {
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = DateConvert.iGetSYearmonthDays(iYear, iMonth);
		if (iDay > iMonthDays) {
	...
	public Week toWeek() {
		...
		for (int i = 1901; i < iYear; i++) {
			if (DateConvert.bIsSolarLeapYear(i))
				iOffsetDays += 366;
			else
				iOffsetDays += 365;
		}
		iOffsetDays += DateConvert.iGetSNewYearOffsetDays(iYear, iMonth, iDay);
	...
	public LunarDate toLunarDate() {
		...
		LunarDate ld;
		iDate = Integer.parseInt(DateConvert.sCalendarSolarToLundar(this.iYear,
...
class LunarDate extends MyDate {
	...
	private static int checkMonth(int iYear, int iMonth) {
		if ((iMonth > 12) && (iMonth == DateConvert.iGetLLeapMonth(iYear) + 12)) {
			return iMonth;
		} else if (iMonth > 12) {
			System.out.println("Month out of range, I think you want 12 ");
			return 12;
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = DateConvert.iGetLMonthDays(iYear, iMonth);
		if (iDay > iMonthDays) {
			System.out.println("Day out of range, I think you want "
					+ iMonthDays + " ");
			return iMonthDays;
	...
	public CnWeek toWeek() {
		int iOffsetDays = 0;
		for (int i = 1901; i < iYear; i++)
			iOffsetDays += DateConvert.iGetLYearDays(i);
		iOffsetDays += DateConvert.iGetLNewYearOffsetDays(iYear, iMonth, iDay);
	...
	public SolarDate toSolarDate() {
		int iYear, iMonth, iDay, iDate;
		SolarDate sd;
		iDate = Integer.parseInt(DateConvert.sCalendarLundarToSolar(this.iYear,
				this.iMonth, this.iDay));
		iYear = iDate / 10000;
		iMonth = iDate % 10000 / 100;
		iDay = iDate % 100;
